...
class RewardModelTest extends TestCase
{
    ...
    public function test_can_purchase_returns_false_when_inactive(): void
    {
        $reward = new Reward(['is_active' => false, 'stock_quantity' => -1]);
        $this->assertFalse($reward->canPurchase());
    }

    public function test_can_purchase_returns_true_when_active_with_unlimited_stock(): void
    {
        $reward = new Reward(['is_active' => true, 'stock_quantity' => -1]);
        $this->assertTrue($reward->canPurchase());
    }

    public function test_can_purchase_returns_true_when_active_with_null_stock(): void
    {
        $reward = new Reward(['is_active' => true, 'stock_quantity' => null]);
        $this->assertTrue($reward->canPurchase());
    }

    public function test_can_purchase_returns_true_when_active_with_positive_stock(): void
    {
        $reward = new Reward(['is_active' => true, 'stock_quantity' => 5]);
        $this->assertTrue($reward->canPurchase());
    }

    public function test_can_purchase_returns_false_when_stock_is_zero(): void
    {
    ...
    public function test_get_rarity_badge_returns_correct_label(string $rarity, string $expected): void
    {
    ...
    public function test_type_s_constant_contains_all_reward_types(): void
    {
        $expected = ['avatar_frame', 'profile_background', 'badge', 'title', 'theme', 'effect'];
        $this->assertSame($expected, array_keys(Reward::TYPES));
    }

    public function test_raritie_s_constant_contains_all_rarity_levels(): void
    {
    ...
    public function test_calculate_test_reward_points_returns_correct_value(
        float $score,
        int $difficulty,
        int $streakDays,
        int $expected
